...
void MapInfo::transferRoadMapInfo(std::vector<std::shared_ptr<t_road> > &vecAllRoad,RoadMapPoint& roadMapPoint)
{
    ...
    m_minPoint = QPointF(0, 0);
    setLaneOffsetOpposite(vecAllRoad);
    setSGeometryOpposite(vecAllRoad);
    getRoadSurfaceContourPoint(vecAllRoad, roadMapPoint);
}

void MapInfo::getRoadGeometryPoint(const RoadMapPoint &roadMapAllPoint, QVector<QVector<QVector<std::pair<QVector<QVector<QPointF> >, int> > > > &qvecQvecQvecQvecQvecQpointf) const
{
...
void MapInfo::getRoadLaneSectionPoint(const RoadMapPoint &roadMapAllPoint, QVector<QVector<QVector<QVector<QPointF> > > > &qvecQvecMapPoint) const
{
    for(int i = 0; i < roadMapAllPoint.size(); ++i)
    {
        // road
...
void MapInfo::getRoadSurfaceContourPoint(const std::vector<std::shared_ptr<t_road> > &vecAllRoad, RoadMapPoint &roadMapAllPoint)
{
    for(auto& eachRoad : vecAllRoad)
    {
        ...
        std::map<int,
                ...
                std::map<std::string,
                std::map<std::string,
                QVector<std::pair<QPointF, int>>>>>> mapEachRoadRoadmarkPoint;


        std::vector<std::shared_ptr<t_road_lanes_laneSection>>& laneSections=eachRoad->m_lanes->m_laneSections;
        for(int i = 0; i < laneSections.size(); ++i)
        {
            LaneSectionPoint laneSectionPoint;
            std::shared_ptr<t_road_lanes_laneSection> eachLaneSection = laneSections[i];
            double dLaneStart = eachLaneSection->s._val;
            double dLaneEnd =getLaneSectionEnd(dRoadLength,dLaneStart,i+1,laneSections);


            std::shared_ptr<t_road_lanes_laneOffset> eachLaneOffset=getLaneOffset(eachLaneSection->s._val,
                eachRoad->m_lanes->m_laneOffsets);

            std::map<int,
                    std::map<std::string,
                    std::map<std::string,
                    QVector<std::pair<QPointF, int>>>>> laneSectionRoadmarkPoint;

        
            laneSectionPoint = getLaneSectionPoint(eachRoad->id, eachLaneOffset, eachLaneSection, eachRoad->m_planView->m_geometrys,
                dLaneStart, dLaneEnd, eachRoad->length._val, laneSectionRoadmarkPoint);
...
static int sgn(double dSgnVar) // 不好用后期修改
{
    int dSgnValue=0;
...
double MapInfo::getLaneSectionEnd(const double& roadLength,const double& dLaneStart,int startSectionIndex,const std::vector<std::shared_ptr<t_road_lanes_laneSection>>& laneSections)
{
    double dLaneEnd = 0.0;
    int j = startSectionIndex;
    for( ; j < laneSections.size(); ++j)
    {
        dLaneEnd = laneSections[j]->s._val;
...
std::shared_ptr<t_road_lanes_laneOffset> MapInfo::getLaneOffset(const double& laneSectionStart,
    const std::vector<std::shared_ptr<t_road_lanes_laneOffset>>& laneOffsets)
{
    std::shared_ptr<t_road_lanes_laneOffset> offset;
    for(const auto &laneOffset : laneOffsets)
    {
        if(laneOffset->s._val > laneSectionStart)
        {
            break;
        }
...
void MapInfo::getAllPointOfLaneFromGeometryS(const std::shared_ptr<t_road_planView_geometry> &geometry, 
    ...
    const double &dGeometryS, const double &dRelatGeometrySStart, const double &dLaneSectionEnd, const double &dRelatLaneSectionSOffset, 
    const double &dRoadLength, const std::vector<double> &vecRoadMarkS, QVector<std::pair<QPointF, int> > &vecQpointf, 
    std::map<int, QVector<std::pair<QPointF, int> > > &qvecQvecLeftLanesPoint, 
    std::map<int, QVector<std::pair<QPointF, int> > > &qvecQvecRightLanesPoint, 
    std::map<std::string, std::map<std::string, QVector<std::pair<QPointF, int> > > > &laneRoadmarkPoint)
{
...
PointFHdg MapInfo::getReferencePointf(const std::shared_ptr<t_road_planView_geometry> &geometry, const std::shared_ptr<t_road_lanes_laneOffset> &laneOffset, const double &dGeometryS, const double &dRelatLaneSectionS) const
{
...
PointFHdg MapInfo::getParamPoly3Data(const std::shared_ptr<t_road_planView_geometry> &geometry, double dPos) const
{
    ...
    double x = geometry->m_t_road_planView_geometry->m_paramPoly3->aU +
            geometry->m_t_road_planView_geometry->m_paramPoly3->bU * pow(pos, 1) +
            geometry->m_t_road_planView_geometry->m_paramPoly3->cU * pow(pos, 2) +
            geometry->m_t_road_planView_geometry->m_paramPoly3->dU * pow(pos, 3);

    double y = geometry->m_t_road_planView_geometry->m_paramPoly3->aV +
            geometry->m_t_road_planView_geometry->m_paramPoly3->bV * pow(pos, 1) +
            geometry->m_t_road_planView_geometry->m_paramPoly3->cV * pow(pos, 2) +
            geometry->m_t_road_planView_geometry->m_paramPoly3->dV * pow(pos, 3);

    double xrot = x * cos(geometry->hdg) - y * sin(geometry->hdg);
    double yrot = x * sin(geometry->hdg) + y * cos(geometry->hdg);

    double dx = 1 * geometry->m_t_road_planView_geometry->m_paramPoly3->bU +
            2 * geometry->m_t_road_planView_geometry->m_paramPoly3->cU * pow(pos, 1) +
            3 * geometry->m_t_road_planView_geometry->m_paramPoly3->dU * pow(pos, 2);
    double dy = 1 * geometry->m_t_road_planView_geometry->m_paramPoly3->bV +
            2 * geometry->m_t_road_planView_geometry->m_paramPoly3->cV * pow(pos, 1) +
            3 * geometry->m_t_road_planView_geometry->m_paramPoly3->dV * pow(pos, 2);
...
void MapInfo::genRoadMarkPointf(QPointF qpointf, double hdg, int iIndexRoadMark, double dRelatLaneSectionS, double dLengthToReferenc,
    ...
    std::map<std::string, std::map<std::string, QVector<std::pair<QPointF, int> > > > &laneRoadmarkPoint)
{
    ...
    {
        if(iIndexRoadMark >= 1 &&
                qvecLane[iLaneIndex]->m_roadMarks[iIndexRoadMark]->type != qvecLane[iLaneIndex]->m_roadMarks[iIndexRoadMark - 1]->type
                && laneRoadmarkPoint[strLaneId]["center"].size() > 0)
        {
            laneRoadmarkPoint[strLaneId]["center"] += std::make_pair(laneRoadmarkPoint[strLaneId]["center"].last().first, iIndexRoadMark);
        }
        else
        {
            if((laneRoadmarkPoint[strLaneId]["center"].size() > 0) &&
                    (laneRoadmarkPoint[strLaneId]["center"].last().second >= 0) &&
                    (laneRoadmarkPoint[strLaneId]["center"].last().second != iIndexRoadMark))
            {
                laneRoadmarkPoint[strLaneId]["center"] += std::make_pair(laneRoadmarkPoint[strLaneId]["center"].last().first, iIndexRoadMark);
            }
        }
...
QVector<std::pair<QPointF, int> > MapInfo::getCurrentLanePoint(const std::shared_ptr<t_road_planView_geometry> &geometry, const PointFHdg &pointfHdg, const int iLaneIndex, const double dLaneSectionEnd, const double dRelatLaneSectionS, const double dRelatRoadS, const double dRoadLength, const std::vector<std::shared_ptr<t_road_lanes_laneSection_lr_lane> > &qvecLane, const std::vector<std::pair<double, PointFHdg> > &qvecRelatLaneSectionSAndPointFHdg, std::map<std::string, std::map<std::string, QVector<std::pair<QPointF, int> > > > &laneRoadmarkPoint, bool isLeft)
{
    ...
    m_maxPoint.setX(qpointf.x() > m_maxPoint.x()? qpointf.x(): m_maxPoint.x());
    m_maxPoint.setY(qpointf.y() > m_maxPoint.y() ? qpointf.y() : m_maxPoint.y());
    m_minPoint.setX(qpointf.x() < m_minPoint.x() ? qpointf.x() : m_minPoint.x());
    m_minPoint.setY(qpointf.y() < m_minPoint.y() ? qpointf.y() : m_minPoint.y());
...
double MapInfo::getDistanceFromCurrentLaneAndS(const int iLaneIndex, const double &dLaneSectionEnd, const double &dLaneSectionDs, 
    const std::vector<std::shared_ptr<t_road_lanes_laneSection_lr_lane>> &vecLane, bool isLeft) const
{
    ...
    {
        if (nullptr == eachWidth.get()) continue;
        double dRelatWidthDs = dLaneSectionDs - eachWidth->sOffset._val;
        dLengthToReferenc += eachWidth->a + eachWidth->b * pow(dRelatWidthDs, 1) +eachWidth->c * pow(dRelatWidthDs, 2) + eachWidth->d * pow(dRelatWidthDs, 3);
    }

    return dLengthToReferenc;
}

std::vector<std::shared_ptr<t_road_lanes_laneSection_lr_lane_width> > MapInfo::getAllWidthToReference(const int iLaneIndex, 
    const double &dLaneSectionEnd, const double &dLaneSectionDs, const std::vector<std::shared_ptr<t_road_lanes_laneSection_lr_lane> > &vecLane, bool isLeft) const
{
...
void MapInfo::getLengthOfWidthFromLane(std::vector<double> &vecWidthLength, std::map<double, int> &mapWidthLengthAndIndex, 
